...
func UsageTemplate() string {
  ...
  {{appendIfNotPresent .Use "[OPTIONS] COMMAND [arg...]\n"}}{{else}}{{.UseLine}}{{end}}{{end}}{{if gt .Aliases 0}}
...
func init() {
	...
	PFlags.StringVarP(&RootCmd.Args.LogLevel, "log-level", "", "info", "set the logging level")
	PFlags.StringVarP(&RootCmd.Args.LogFile, "log", "l", "/var/log/jwt.log", "path to use for log file")
